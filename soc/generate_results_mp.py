...
def compute_objectives(approval_sizes, utilities, minority_projects, majority_projects, setup, output_queue):
    ...
    if 'final_' in setup:
        setup = setup[6:]
    output_queue.put(setup,objectives,cc_approvals_mean)
...
def simulate_wrapper(output_queue):
    r = simulate_for_all_group_divs()
    dicobj = json.dumps(r).encode('utf-8')
    output_queue.put(dicobj)

def generate_results_exp():
    objectives_results = {}
    ...
    while x < num_simulations:
        processes = [mp.Process(target=simulate_wrapper,args=(simulate_queue,)) for i in range(min(num_simulations-x,batch_size))]
        for p in processes:
            p.start()
        ...
        results += [json.loads(simulate_queue.get()) for p in processes]
        x += batch_size
        print('Progress: {}/{}'.format(x,num_simulations))
    assert len(results) == num_simulations
    for opinions in results:
    ...
    for r in results:
        objectives_results[r[0]] = r[1]
        cc_approvals_results[r[0]] = r[2]
