...
def compute_objectives(approval_sizes, utilities, minority_projects, majority_projects, setup, output_queue):
    ...
    for n in range(num_simulations):
        ...
        for rule_id in rules:
            ...
            violation_results = perc_satisfaction(profile.profile_abc, rule_id, list(result))
            objectives['violationsx'][rule_id] += violation_results[1]
            objectives['violationsy'][rule_id] += violation_results[2]
            objectives['cohesive_groups'][rule_id].append(violation_results[3])
            objectives['violation_percent'][rule_id].append(len(violation_results[1])/violation_results[3])
